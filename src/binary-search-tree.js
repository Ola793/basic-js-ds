...
class BinarySearchTree {
  ...
  add(data) {
    function addNewNumber(node, data) {
      if (!node) {
        return new Node(data);
      } 

      data > node.data ? node.right = addNewNumber(node.right, data) : node.left = addNewNumber(node.left, data);

      return node;
    }

    return this.rootNumber = addNewNumber(this.rootNumber, data);
  }
